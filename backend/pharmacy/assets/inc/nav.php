<?php
    ...
    $pharm_email = $_SESSION['pharm_email'];
    $ret="SELECT * FROM  his_pharm WHERE pharm_id = ? AND pharm_email = ?";
    $stmt= $mysqli->prepare($ret) ;
    $stmt->bind_param('is', $pharm_id, $pharm_email);
    $stmt->execute() ;//ok
    $res=$stmt->get_result();
    //$cnt=1;
    ...
    <div class="d-flex align-items-center">
        ...
        <div class="dropdown d-none d-lg-block">
            <button class="btn btn-link text-dark font-weight-bold text-decoration-none dropdown-toggle px-3 py-2 rounded-pill hover:bg-light transition-all" 
                    data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false" style="font-size: 14px;">
               Create New
    ...
    <div class="d-flex align-items-center">
        ...
        <div class="dropdown">
            <button class="btn d-flex align-items-center p-1 rounded-pill hover:bg-light transition-all dropdown-toggle border-0" 
                ...
                <div class="text-right mr-2 d-none d-sm-block">
                    <p class="mb-0 font-weight-bold text-dark" style="font-size: 13px; line-height: 1;">
                        <?php echo $row->pharm_fname . ' ' . $row->pharm_lname; ?>
                    </p>
                    <small class="text-muted font-weight-medium">Pharmacist</small>
                </div>
            ...
            <div class="dropdown-menu dropdown-menu-right border-0 shadow-lg rounded-xl py-2 mt-2" style="min-width: 200px;">
                ...
                <a href="his_pharm_logout_partial.php" class="dropdown-item d-flex align-items-center px-4 py-2 text-danger font-weight-bold">
