                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-xl-7">
                                                <div class="pl-xl-3 mt-3 mt-xl-0">
                                                    ...
                                                    <hr>
                                                    <?php
                                                        // Show related entries (author-stamped records) for this prescription
                                                        $entries = get_entries_by_entity($mysqli, 'prescription', $row->pres_id, 20);
                                                        if (!empty($entries)) {
                                                            echo '<h5>Audit / Notes</h5>';
                                                            echo '<ul class="list-group">';
                                                            foreach ($entries as $e) {
                                                                $author = htmlspecialchars($e['author_name'] ?? 'Unknown');
                                                                $when = htmlspecialchars($e['created_at']);
                                                                $content = $e['content'];
                                                                echo "<li class=\"list-group-item\"><div><strong>{$author}</strong> <small class=\"text-muted\">{$when}</small></div><div class=\"mt-2\">{$content}</div></li>";
                                                            }
                                                            echo '</ul>';
                                                        }
                                                    ?>
