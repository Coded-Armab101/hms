...
// Fetch available pharmaceuticals from inventory
// Using his_pharmaceuticals table with quantity > 0
$pharm_query = "SELECT * FROM his_pharmaceuticals WHERE phar_qty > 0 ORDER BY phar_name ASC";
$pharm_stmt = $mysqli->prepare($pharm_query);
$pharm_stmt->execute();
...
<body>
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <table class="table table-borderless table-sm">
                                            ...
                                            <tr>
                                                ...
                                                <td><?php echo date('d M Y', strtotime($pres_details->created_at ?? date('Y-m-d'))); ?></td>
                                            </tr>
                                        </table>
                                    </div>
                                    <div class="col-md-6">
                                        ...
                                        <div class="p-3 border rounded">
                                            <?php echo nl2br(htmlspecialchars($pres_details->prescription_text ?? 'No prescription details')); ?>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                        ...
                                        <div class="drug-entry card mb-3" id="entry1">
                                            <div class="card-body">
                                                ...
                                                <div class="row">
                                                    <div class="col-md-5">
                                                        <div class="form-group">
                                                            <label>Select Pharmaceutical <span class="text-danger">*</span></label>
                                                            <select name="phar_id[]" class="form-control pharm-select" required onchange="updateStockInfo(this)">
                                                                <option value="">-- Select Drug --</option>
                                                                <?php 
                                                                if($pharm_res->num_rows > 0) {
                                                                    while($pharm = $pharm_res->fetch_object()) { 
                                                                ?>
                                                                <option value="<?php echo $pharm->phar_id; ?>" 
                                                                        data-stock="<?php echo $pharm->phar_qty; ?>"
                                                                        data-price="<?php echo $pharm->phar_price_unit; ?>"
                                                                        data-name="<?php echo htmlspecialchars($pharm->phar_name); ?>"
                                                                        data-unit="<?php echo htmlspecialchars($pharm->phar_unit); ?>"
                                                                        data-attribute="<?php echo htmlspecialchars($pharm->phar_attribute); ?>">
                                                                    <?php echo htmlspecialchars($pharm->phar_name); ?> 
                                                                    (<?php echo htmlspecialchars($pharm->phar_attribute ?? 'N/A'); ?>, 
                                                                    Stock: <?php echo $pharm->phar_qty; ?>, 
                                                                    Price: $<?php echo number_format($pharm->phar_price_unit, 2); ?>)
                                                                </option>
                                                                <?php 
                                                                    }
                                                                } else {
                                                                ?>
                                                                <option value="">No pharmaceuticals available in stock</option>
                                                                <?php } ?>
                                                            </select>
                                                        </div>
                                                    </div>
                                                ...
                                                <div class="row">
                                                    ...
                                                    <div class="col-md-4">
                                                        <small class="text-muted unit-info">Unit/Package: -</small>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
    ...
    $(document).ready(function() {
        ...
        $("#addMoreDrugs").click(function() {
            ...
            newEntry.find('.unit-info').text('Unit/Package: -');
        ...
        window.updateStockInfo = function(selectElement) {
            ...
            let attribute = selected.data('attribute');
            ...
            entry.find('.unit-info').text('Unit/Package: ' + (unit || 'N/A') + ' ' + (attribute || ''));
        };
        ...
        window.checkStock = function(inputElement) {
            ...
            let select = entry.find('.pharm-select');
            let selected = select.find(':selected');
            let stock = parseInt(selected.data('stock')) || 0;
            
            if(quantity > stock) {
                alert('Warning: Requested quantity exceeds available stock! Available stock: ' + stock);
