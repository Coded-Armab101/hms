...
<?php
    ...
    include(__DIR__ . '/../inc/entry_helper.php');
		if(isset($_POST['add_patient_presc']))
		{
            ...
            // If insert succeeded, create a DRY entry record linking to the prescription
            if ($ok) {
                $pres_id = $mysqli->insert_id;
                $author_id = isset($_SESSION['ad_id']) ? $_SESSION['ad_id'] : null;
                $author_name = isset($_SESSION['ad_name']) ? $_SESSION['ad_name'] : null;
                $author_role = isset($_SESSION['role']) ? $_SESSION['role'] : null;
                // title can be prescription number or patient name
                $title = !empty($pres_number) ? 'Prescription: ' . $pres_number : 'Prescription';
                // create_entry returns the new entry id or false
                $entry_id = create_entry($mysqli, 'prescription', $pres_id, $title, $pres_ins, $author_id, $author_name, $author_role);
            }
			/*
			...
			*/ 
            ...
            if($ok)
            {
                $success = "Patient Prescription Added";
            }
            else {
